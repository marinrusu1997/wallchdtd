...
import { computeWallpaperPath } from '../wallpaper/path.js';

const schema = {
	type: 'object',
	properties: {
		changeBy: {
			enum: ['sunlight', 'daytime']
		},
		wallpapersDir: {
			type: 'string',
			minLength: 1
...
function validate(config) {
	...
	for (const [time, wallpaper] of Object.entries(config.wallpapers)) {
		if (time === '24:00' || time === '00:00') {
			throw new Error(`Midnight time '${time}' is not allowed. Please specify either '23:59', or '00:01'.`);
		}
		if (computeWallpaperPath(wallpaper, config.wallpapersDir) == null) {
